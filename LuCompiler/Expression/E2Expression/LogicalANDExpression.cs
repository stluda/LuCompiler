...
{
    public class LogicalANDExpression : E2Expression
    {
        public override string getCombinedValue(string value1, string value2)
        {
            if (E1 is LogicalORExpression) value1 = string.Format("({0})", value1);
            if (E2 is LogicalORExpression) value2 = string.Format("({0})", value2);
            return base.getCombinedValue(value1, value2);
        }

        public LogicalANDExpression(Expression e1, Expression e2) 
            : base(e1,e2,Symbol.GetInstance("&&", typeof(LogicalANDExpression)))
        {        
        }
        ...
        public override string QTranslate(out string prefixStatement, out bool interValFlag, out string postfixStatement)
        {
            ...
            if (flag2 || E2 is PostfixExpression && (E2 as PostfixExpression).HasPostfixExpression)
            {
                ...
                sb.Append(string.Format("{0}Dim {1} : ", statement1, returnValue))
                    .Append(string.Format("{0} = False : ", returnValue))
                    .Append(string.Format("If {0} Then : {1}{2} = {3} : End If : ", value1
